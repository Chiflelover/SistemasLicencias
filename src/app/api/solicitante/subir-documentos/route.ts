...
export async function POST(request: Request) {
  try {
    ...
    if (!(rawFile instanceof File)) {
      return NextResponse.json({ error: "No se envió un archivo válido." }, { status: 400 });
    }
    ...
    if (rawFile.size > MAX_FILE_SIZE) {
      return NextResponse.json({ error: "El archivo no puede ser mayor a 5MB." }, { status: 400 });
    }
    ...
    return NextResponse.json({ success: true, document: documentWithoutContent });
  } catch (error: any) {
    return NextResponse.json(
