    ...
    <div className="space-y-8 animate-fadeIn">
      ...
      <div className="bg-slate-900/40 border border-slate-850 rounded-3xl overflow-hidden">
        <div className="p-6 border-b border-slate-850">
          ...
          <p className="text-slate-400 text-sm mt-1">Aquí se muestran las multas registradas por ti.</p>
        </div>
            ...
            <tbody className="divide-y divide-slate-850/80">
              {fines.map((fine) => (
                <tr key={fine.id} className="hover:bg-slate-900/20">
                  <td className="px-4 py-4 text-white">{fine.license.licenseNumber}</td>
                  <td className="px-4 py-4">
                    <div>{fine.license.application.business.legalName}</div>
                    <div className="text-xs text-slate-500">{fine.license.application.business.ruc}</div>
                  </td>
                  <td className="px-4 py-4 text-amber-300">S/ {fine.amount.toFixed(2)}</td>
                  <td className="px-4 py-4 text-slate-400">{new Date(fine.issuedAt).toLocaleDateString("es-PE")}</td>
                  <td className="px-4 py-4 text-slate-200">{fine.status}</td>
                </tr>
              ))}
