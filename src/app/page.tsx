...
import { Shield, ArrowRight, ClipboardCheck, Sparkles, HelpCircle } from "lucide-react";
    ...
    <main className="min-h-screen bg-slate-950 text-slate-100 flex flex-col justify-between selection:bg-amber-500 selection:text-slate-950">
      ...
      <header className="border-b border-slate-800 bg-slate-900/40 backdrop-blur-md sticky top-0 z-50">
        <div className="max-w-7xl mx-auto px-6 py-4 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <span className="font-bold text-white tracking-wide block">MUNICIPALIDAD DE TRUJILLO</span>
              <span className="text-slate-500 text-xs uppercase font-medium">Ciudad de la Primavera</span>
            </div>
          </div>
      ...
      <section className="flex-grow max-w-7xl mx-auto px-6 py-12 md:py-20 flex flex-col lg:flex-row items-center justify-between gap-12">
        <div className="space-y-6 lg:w-1/2">
          ...
          <h1 className="text-4xl md:text-5xl lg:text-6xl font-extrabold text-white tracking-tight leading-none">
            Licencias Municipales de <span className="text-amber-400">Funcionamiento</span>
          </h1>
          <p className="text-slate-400 text-lg leading-relaxed max-w-lg">
            Solicita, tramita, y obtén la licencia comercial de funcionamiento para tu establecimiento en Trujillo de forma 100% digital, rápida y transparente.
          </p>
        ...
        <div className="lg:w-1/2 w-full grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div className="bg-slate-900/50 border border-slate-850 p-6 rounded-2xl space-y-3">
            ...
            <p className="text-slate-400 text-sm">
              Tus datos personales y de negocio están protegidos con autenticación segura y cifrado.
            </p>
          </div>
          ...
          <div className="bg-slate-900/50 border border-slate-850 p-6 rounded-2xl space-y-3">
            ...
            <h3 className="font-bold text-white text-lg">Requisitos Simples</h3>
            <p className="text-slate-400 text-sm">
              Solo necesitas tu RUC de 11 dígitos, planos del establecimiento en PDF/imagen y abonar S/ 2.
            </p>
          </div>

          <div className="bg-slate-900/50 border border-slate-850 p-6 rounded-2xl space-y-3 sm:col-span-2">
            <div className="flex justify-between items-start">
              <div className="space-y-1">
                <h3 className="font-bold text-white text-lg">Costo del Trámite</h3>
                <p className="text-slate-400 text-sm">
                  De acuerdo con el Texto Único de Procedimientos Administrativos (TUPA) vigente.
                </p>
              </div>
      ...
      <footer className="border-t border-slate-900 bg-slate-950 py-8 text-center text-slate-500 text-sm">
        <div className="max-w-7xl mx-auto px-6 flex flex-col md:flex-row justify-between items-center gap-4">
          <p>© {new Date().getFullYear()} Municipalidad Provincial de Trujillo. Todos los derechos reservados.</p>
          <div className="flex gap-4 text-xs">
            <span className="hover:text-slate-300 transition cursor-help">Términos</span>
            <span className="hover:text-slate-300 transition cursor-help">Privacidad</span>
            <span className="hover:text-slate-300 transition cursor-help">Soporte TUPA</span>
          </div>
        </div>
      </footer>
    </main>
